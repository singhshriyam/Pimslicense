...
const ResolvedIncidents: React.FC<ResolvedIncidentsProps> = ({
  userType,
  onBack,
  initialStatusFilter
}) => {
  ...
  // Data Fetching - Filter for resolved, closed and cancelled incidents (4, 5, 6)
  const fetchData = async () => {
    try {
      ...
      // Filter for resolved, closed and cancelled incidents (4=Resolved, 5=Closed, 6=Cancelled)
      const resolvedClosedIncidents = fetchedIncidents.filter(incident => {
        return incident.incidentstate_id === 4 || incident.incidentstate_id === 5 || incident.incidentstate_id === 6
      })

      resolvedClosedIncidents.sort((a, b) =>
        new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
      )

      setIncidents(resolvedClosedIncidents)
      setFilteredIncidents(resolvedClosedIncidents)
      setCurrentPage(1)

    } catch (error: any) {
      setError(error.message || 'Failed to load resolved incidents')
      setIncidents([])
      ...
      <Container fluid>
        <div className="text-center py-5">
          <div className="spinner-border text-primary" role="status">
            <span className="visually-hidden">Loading resolved incidents...</span>
          </div>
          <p className="mt-3 text-muted">Loading resolved incidents...</p>
        </div>
      </Container>
    )
  }
            ...
            <Card className="mb-4 mt-4">
              <CardBody>
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    <h4 className="mb-1">Resolved & Closed Incidents</h4>
                    <small className="text-muted">
                      View all resolved, closed and cancelled incidents ({filteredIncidents.length} total)
                    </small>
                  </div>
                  <div>
                    <Button
        ...
        <Row>
          <Col xs={12}>
            <Card>
              <CardHeader>
                <div className="d-flex justify-content-between align-items-center">
                  <h5>Filter Resolved Incidents</h5>
                  <Button
                    color="outline-primary"
                    size="sm"
                    onClick={fetchData}
          ...
          <Col xs={12}>
            <Card>
              <CardHeader>
                <div className="d-flex justify-content-between align-items-center">
                  <h5>
                    {hasFullEditPermissions() ? 'All Resolved & Closed Incidents' : 'My Resolved & Closed Incidents'} ({filteredIncidents.length})
                  </h5>
                  <div>
                    <small className="text-muted">
              ...
              <CardBody>
                {filteredIncidents.length === 0 ? (
                  <div className="text-center py-4">
                    <h6 className="text-muted">No resolved or closed incidents found</h6>
                    <p className="text-muted">No resolved or closed incidents match your current criteria.</p>
                            ...
                            <tr key={incident.id}>
                              ...
                              <td>
                                <small className="text-dark">
                                  {formatDateOnly(incident.closed_at || incident.updated_at)}
                                </small>
                              </td>
                              <td>
