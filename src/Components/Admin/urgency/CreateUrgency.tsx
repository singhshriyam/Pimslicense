...
if (result.isConfirmed) {
    try {
      ...
      const filterImpacts = impacts.filter((i: any) => i.id !== id);
      setImpacts(filterImpacts);
    } catch (error) {}
  }
  });
  ...
  const getImpacts = async () => {
    try {
      const response = await axios.get(
        "https://apexwpc.apextechno.co.uk/api/master/impacts",
          ...
          if (response.status === 200) {
           getImpacts();
            ...
            Swal.fire({
              icon: "success",
              title: "Success!",
              text: "Impact   Created Successfully!",
            });
          }
        } catch (error: any) {
            ...
            <div className="row">
              <div className="col-md-4">
                <div className="form-group">
                  <label htmlFor="">Impact Name <span className="text-danger">*</span></label>
                  <input
                ...
                <th colSpan={2}>Action</th>
                    ...
                    <tbody>
                      {impacts.map((impact:any) => (
                        <tr key={impact.id}>
                          <td>
                            <span className="fw-medium text-primary">{impact.id}</span>
                          </td>
                          <td>
                           {impact.name} 
                          </td>
                          ...
                          href={`/admin/contact-type/${impact.id}`}
                        >
                         ✎
                        </Link>
                              <Button
