...
const CompanySchema = Yup.object({
  ...
  name: Yup.string().required("Please enter site type name"),
 });
const API_BASE_URL = process.env.API_BASE_URL;
const token = localStorage.getItem("authToken");

const CreateAssetCompany = () => {
...
if (result.isConfirmed) {
    try {
      ...
      const filterAssetCompanies = assetCompanies.filter((assetCompany: any) => assetCompany.id !== id);
      setAssetCompanies(filterAssetCompanies);
    } catch (error) {}
  }
  });
          ...
          if (response.status === 200) {
           getAssetCompanies();
            ...
            Swal.fire({
              icon: "success",
              title: "Success!",
              text: "Asset Company creates Successfully!",
            });
          }
        } catch (error: any) {
            ...
            <div className="row">
              <div className="col-md-4">
                <div className="form-group">
                  <label htmlFor="">Site Type Name <span className="text-danger">*</span></label>
                  <input
            ...
            <CardHeader>
              <div className="d-flex justify-content-between align-items-center">
                ...
                <Badge color="danger" className="fs-6">
                  {assetCompanies.length} Asset Company
                </Badge>
              </div>
            </CardHeader>
            <CardBody>
              {assetCompanies.length > 0 ? (
                ...
                <th colSpan={2}>Action</th>
                    ...
                    <tbody>
                      {assetCompanies.map((siteType:any) => (
                        <tr key={siteType.id}>
                          <td>
                            <span className="fw-medium text-primary">{siteType.id}</span>
                          </td>
                          <td>
                           {siteType.name} 
                          </td>
                          ...
                          href={`/admin/site-type/${siteType.id}`}
                        >
                         ✎
                        </Link>
                              <Button
                ...
                <div className="text-center py-5">
                  <p className="text-muted mb-0">No SiteTYpe</p>
...
export default CreateAssetCompany;
